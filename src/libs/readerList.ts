const readerList = (lists: any[], count: number, page: number, pageSize: number, cate?: number | string, searchVal?: string) => {
    let html = !searchVal ? `` : `<div class="text-2xl font-bold col-span-3 mb-2">Search "${searchVal}" Results</div>`;
    ...
    if (count > (page * pageSize)) {
        html += `<div class="col-span-3 text-center">
                    <button class="more font-bold text-green-600 underline dark:text-green-400" data-index="${page + 1}" data-cate="${cate}" data-val="${searchVal}">Load More</button>
                </div>}`
    }
    return html;
}
